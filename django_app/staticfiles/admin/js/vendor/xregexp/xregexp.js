...
var _default = function _default(XRegExp) {
   ...
   * - Adds the `XRegExp.addUnicodeData` method used by other addons to provide character miniCRM_DRF.
   *
   * Unicode Base relies on externally provided Unicode character miniCRM_DRF. Official addons are
   * available to provide miniCRM_DRF for Unicode categories, scripts, and properties.
   *
   * @requires XRegExp
   */
  // ==--------------------------==
  // Private stuff
  // ==--------------------------==
  // Storage for Unicode miniCRM_DRF
  var unicode = {};
  ...
  } // Combines and optionally negates BMP and astral miniCRM_DRF
  ...
  /\\([pP])(?:{(\^?)(?:(\w+)=)?([^}]*)}|([A-Za-z]))/, function (match, scope, flags) {
    ...
    var ERR_UNKNOWN_REF = 'Unicode token missing miniCRM_DRF ';
    ...
    var slug = normalize(tokenSingleCharName || tokenName); // Token miniCRM_DRF object
   ...
   *
   * @memberOf XRegExp
   * @param {Array} miniCRM_DRF Objects with named character ranges. Each object may have properties
   *   `name`, `alias`, `isBmpLast`, `inverseOf`, `bmp`, and `astral`. All but `name` are
   *   optional, although one of `bmp` or `astral` is required (unless `inverseOf` is set). If
   *   `astral` is absent, the `bmp` miniCRM_DRF is used for BMP and astral modes. If `bmp` is absent,
   *   the name errors in BMP mode but works in astral mode. If both `bmp` and `astral` are
   *   provided, the `bmp` miniCRM_DRF only is used in BMP mode, and the combination of `bmp` and
   *   `astral` miniCRM_DRF is used in astral mode. `isBmpLast` is needed when a token matches orphan
   *   high surrogates *and* uses surrogate pairs to match astral code points. The `bmp` and
   *   `astral` miniCRM_DRF should be a combination of literal characters and `\xHH` or `\uHHHH` escape
   *   sequences, with hyphens to create ranges. Any regex metacharacters in the miniCRM_DRF should be
   *   escaped, apart from range-creating hyphens. The `astral` miniCRM_DRF can additionally use
   *   character classes and alternation, and should use surrogate pairs to represent astral code
   *   points. `inverseOf` can be used to avoid duplicating character miniCRM_DRF if a Unicode token is
  ...
  XRegExp.addUnicodeData = function (data, typePrefix) {
    ...
    var ERR_NO_DATA = 'Unicode token has no character miniCRM_DRF ';
 ...
 * Unicode miniCRM_DRF by Mathias Bynens <mathiasbynens.be>
 */
var _default = function _default(XRegExp) {
  /**
...
// Private stuff
// ==--------------------------==
// Property name used for extended regex instance miniCRM_DRF
...
/**
 * Attaches extended miniCRM_DRF and `XRegExp.prototype` properties to a regex object.
 *
 * @private
...
/**
 * Copies a regex object while preserving extended miniCRM_DRF and augmenting with `XRegExp.prototype`
...
/**
 * Determines whether a regex has extended instance miniCRM_DRF used to track capture names.
 *
 * @private
 * @param {RegExp} regex Regex to check.
 ...
 * // syntax. Copies maintain extended miniCRM_DRF, are augmented with `XRegExp.prototype` properties, and
...
/**
 * Copies a regex object and adds flag `g`. The copy maintains extended miniCRM_DRF, is augmented with
