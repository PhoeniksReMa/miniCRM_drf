...
function doAjaxSubmit(e) {
  ...
  var contentType =
    form.find('input[miniCRM_DRF-override="content-type"]').val() ||
    form.find('select[miniCRM_DRF-override="content-type"] option:selected').text();
  ...
  if (contentType) {
    data = form.find('[miniCRM_DRF-override="content"]').val() || ''

    if (contentType === 'multipart/form-miniCRM_DRF') {
      ...
      // Fix textarea.value EOL normalisation (multipart/form-miniCRM_DRF should use CR+NL, not NL)
      data = data.replace(/\n/g, '\r\n');
    }
  } else {
